...
class XtractorCommand(Subcommand):
    ...
    def xtract(self):
        ...
        self._say("Number of items to be processed: {}".format(
            len(self.items_to_analyse)))
        ...
        if self.cfg_items_per_run != 0:
            self.items_to_analyse = self.items_to_analyse[
                                    :self.cfg_items_per_run]
        self._say(
            "Number of items selected: {}".format(len(self.items_to_analyse)))

        # Run tasks on selected items
        self._execute_on_each_items(self.items_to_analyse,
                                    self.run_full_analysis)

        # Delete profiles (if config wants)
        if self.config["keep_profiles"].exists() and not self.config[
            "keep_profiles"].get():
        ...
        if not self.cfg_force:
            ...
            for map_key in target_maps:
                ...
                for fld in target_map:
                    if target_map[fld]["required"].exists() and target_map[fld][
                        "required"].get(bool):
                        fast = fld in Item._fields
                        query_item = dbcore.query.MatchQuery(fld, None,
                                                             fast=fast)
    ...
    def run_full_analysis(self, item):
        ...
        if self.config["keep_output"].exists() and not self.config[
            "keep_output"].get():
